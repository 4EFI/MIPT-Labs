...
def read_excel(filename, usecols, header = None, nrows = None, sheet_name = 0):
    ...
    if header == None or type(header) == int or len(header) == 1:
        return pd.read_excel(filename, usecols = usecols, header = header, nrows = nrows, sheet_name = sheet_name)

    # Read header rows
    __header_df = pd.read_excel(filename, nrows = len(header), header = header, sheet_name = sheet_name)
    __header_mi = pd.MultiIndex.from_tuples(__header_df.columns.to_list())

    # Read data and set multiindex
    __data = pd.read_excel(filename, skiprows = header[-1] + 1, usecols = usecols, nrows = nrows, sheet_name = sheet_name)
